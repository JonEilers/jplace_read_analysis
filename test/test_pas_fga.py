from src.pas_fga import tree_splitter, edge_counter, number_of_placements, placement_location, edge_indice

fake_tree = {'tree': '(root{0},bob{1},bill|c|{3})',
             'placements':[
...
def test_tree_splitter():
    ...
    assert tree_splitter(fake_tree) == ['','root{0}', 'bob{1}', 'bill|c|{3}', '']
...
def test_edge_counter():
    ...
    assert results['internalEdges'] == [0,1]
